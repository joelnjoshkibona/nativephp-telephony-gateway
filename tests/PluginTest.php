...
/**
 * Plugin validation tests for TelephonyGateway.
 *
...
describe('Plugin Manifest', function () {
    ...
    it('has required fields', function () {
        ...
        expect($manifest['name'])->toBe('blutrixx/nativephp-telephony-gateway');
        expect($manifest['namespace'])->toBe('TelephonyGateway');
    });
...
describe('Native Code', function () {
    it('has Android Kotlin file', function () {
        $kotlinFile = $this->pluginPath . '/resources/android/TelephonyGatewayFunctions.kt';

        expect(file_exists($kotlinFile))->toBeTrue();

        $content = file_get_contents($kotlinFile);
        expect($content)->toContain('package com.blutrixx.plugins.nativephp_telephony_gateway');
        expect($content)->toContain('object TelephonyGatewayFunctions');
        expect($content)->toContain('BridgeFunction');
    });

    it('has iOS Swift file', function () {
        $swiftFile = $this->pluginPath . '/resources/ios/TelephonyGatewayFunctions.swift';

        expect(file_exists($swiftFile))->toBeTrue();

        $content = file_get_contents($swiftFile);
        expect($content)->toContain('enum TelephonyGatewayFunctions');
        expect($content)->toContain('BridgeFunction');
    });
...
describe('PHP Classes', function () {
    it('has service provider', function () {
        $file = $this->pluginPath . '/src/TelephonyGatewayServiceProvider.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Blutrixx\NativephpTelephonyGateway');
        expect($content)->toContain('class TelephonyGatewayServiceProvider');
    });

    it('has facade', function () {
        $file = $this->pluginPath . '/src/Facades/TelephonyGateway.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Blutrixx\NativephpTelephonyGateway\Facades');
        expect($content)->toContain('class TelephonyGateway extends Facade');
    });

    it('has main implementation class', function () {
        $file = $this->pluginPath . '/src/TelephonyGateway.php';
        expect(file_exists($file))->toBeTrue();

        $content = file_get_contents($file);
        expect($content)->toContain('namespace Blutrixx\NativephpTelephonyGateway');
        expect($content)->toContain('class TelephonyGateway');
    });
});
